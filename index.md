...
当时方案里并不是只设计了一条路径，而是留了几种层次不同的做法。核心问题并不是“能不能做标定”，而是：**哪条标定链最稳定、最符合传感器本身的观测条件。**

### 4.1 RGB–D 联合标定

最直接的思路，是利用棋盘格、平面模型和对应点，拟合：
...
$$
\Pi_i:\quad n_i^\top X + d_i = 0,
$$

其中 $n_i$ 是平面法向量，$d_i$ 是平面偏移。对于 RGB 相机中的棋盘格角点，采用标准针孔投影模型：
...
这里 $(X_w,Y_w,0)$ 是标定板平面坐标系中的角点位置。RGB 相机内参与每一帧的外参可以通过成熟的棋盘格标定方法稳定估计。

对深度相机而言，情况更复杂。若深度图像素为 $u_d=(u,v,1)^\top$，其测得深度值为 $D(u,v)$，则可先通过深度尺度关系恢复三维点

$$
z = a\,D(u,v) + b,
$$

再由内参逆映射得到深度相机坐标系中的三维点

$$
X_d = z\,K_d^{-1}u_d.
$$

如果该点对应于标定板平面，那么它在世界坐标系中应满足平面约束。设深度相机到世界系的外参为 $(R_i^d,t_i^d)$，则有
...
于是可以把深度相机的标定问题写成一个几何一致性优化问题：既要使 RGB 图像上的角点重投影误差足够小，也要使深度恢复出的三维点尽可能落在对应的标定平面上。一个自然的联合目标可以写成
...
x_{ij}^{rgb}
-
\pi\!\left(K_{rgb},R_i^{rgb},t_i^{rgb},X_j\right)
\right\|^2
+
\lambda
\sum_{i,k}
\left(
n_i^\top (R_i^d X_{ik}^d + t_i^d) + d_i
\right)^2,
$$

其中第一项是 RGB 角点重投影误差，第二项是深度点到平面的几何残差，$\Theta$ 表示所有待估参数，$\lambda$ 是平衡系数。

这样写的好处是，它把 RGB 与 D 的观测统一到了同一套几何约束里，而不是仅仅依赖“图像间看起来对齐”。

这里有一个很现实的判断：**深度图并不天然适合直接作为高精度棋盘格角点检测的入口。**  
它会有孔洞、跳变和不稳定区域，所以如果直接在 Depth 图上强行做角点级初始化，往往很吃力。

因此在这类标定问题里，平面模型之所以重要，不是因为它“简单”，而是因为它恰好提供了一种对深度观测更稳的几何参照：
...
原因很简单：在很多设备上，IR 图像对棋盘格角点的呈现比深度图自然得多。于是如果 IR 与 D 在设备内部本来就具有稳定的几何对应关系，那么完全可以转而先走 RGB–IR 双目标定，再间接约束 D。

设 RGB 与 IR 之间的相对位姿为 $(R_{ir\leftarrow rgb}, t_{ir\leftarrow rgb})$，则对同一个空间点 $X$ 有

$$
x_{rgb} \sim K_{rgb}[I\;0]X,
\qquad
x_{ir} \sim K_{ir}[R_{ir\leftarrow rgb}\; t_{ir\leftarrow rgb}]X.
$$

于是 RGB–IR 标定可以直接按双目标定问题处理，其目标函数就是标准的双视角重投影误差最小化：
...
\sum_j
\left\|
x_j^{rgb} - \pi(K_{rgb},X_j)
\right\|^2
+
\mu
\sum_j
\left\|
x_j^{ir} - \pi(K_{ir},R_{ir\leftarrow rgb},t_{ir\leftarrow rgb},X_j)
\right\|^2.
$$

这条路线的关键不在于公式更复杂，而在于观测本身更稳定：  
如果 IR 图像能更清楚地提供棋盘格角点，那么它就比 Depth 图更适合作为标定入口。此时，Depth 不再承担“直接识别棋盘格”的任务，而更多承担“接受几何约束”的角色。
...
从系统设计角度看，这其实揭示了一个很重要的判断：

> **在底层视觉系统里，好的方案不一定是“最直观”的链条，而是“最稳定的几何中介”。**

对于这类 RGB–D–IR 设备，与其在深度图上强行完成全部初始化，不如利用 IR 提供更稳定的角点观测，再把这种稳定性传递到深度相机几何上。这样做的结果，往往比直接围绕 Depth 图做高精度初始化更自然，也更稳。
